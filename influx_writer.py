...
async def write_sagittal_frame(rep_number: int, frame_number: int,
                                   ...
                                   heel_lift_percent: float,
                                   patient_name: str, patient_id: str, session_id: str) -> None:
        """
...
async def write_session_summary(squat_count: int, max_angle: float, fms_score: int, patient_name: str, patient_id: str, session_id: str, video_url: str, view_mode: str = "sagittal") -> None:
    """Write the session summary with video URL for the history dashboard."""
    try:
        async with InfluxDBClientAsync(
                url=INFLUX_URL, token=INFLUX_TOKEN, org=INFLUX_ORG
        ) as client:
            point = (
                ...
                .tag("patient_id", str(patient_id))
                .tag("session_id", str(session_id))
                .tag("view_mode", view_mode)
                .field("squat_count", int(squat_count))
                .field("max_angle", float(max_angle))
                .field("fms_score", int(fms_score))
                .field("video_url", video_url)
            )
            write_api = client.write_api()
            await write_api.write(bucket=INFLUX_BUCKET, record=point)
    except Exception as e:
        print(f"[InfluxDB] session summary write error: {e}")
